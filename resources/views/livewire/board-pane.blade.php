...
<tr>
    ...
    <td class="px-6 py-4 whitespace-nowrap">
        @if(!$editable)
        <a href="{{ route('board.show', $board->id) }}" class="text-sm font-medium bg-white text-gray-900">{{ $title }}</a>
        @else
        <input wire:model="title" value="{{ $title }}" class="text-sm font-medium bg-white text-gray-900 @if($editable) border-2 border-r-2 @endif" @if(!$editable) disabled @endif />
        @endif
    </td>
